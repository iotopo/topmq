...
async function loadDialogChart() {
  const res = await metricsService.query<MessagesSeries>({
    metric: 'mqtt_server_messages',
...
function getChartOption(res: MessagesSeries[]) {
  return {
    tooltip: {
      trigger: 'axis',
    ...
    series: [
      {
        ...
        data: res.map(item => [item.time, item.messages_received]),
      },
      {
        type: 'line',
        name: `messages_sent`,
        ...
        data: res.map(item => [item.time, item.messages_sent]),
      },
      {
        type: 'line',
        ...
        data: res.map(item => [item.time, item.messages_dropped]),
      },
    ],
  }
}
