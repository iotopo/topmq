...
// RedisOptions contains configuration settings for the bolt instance.
...
// RedisStorageHook is a persistent storage hook based using Redis as a backend.
type RedisStorageHook struct {
	mqtt.HookBase
	config *RedisOptions         // options for connecting to the Redis instance.
	db     redis.UniversalClient // the Redis instance
	ctx    context.Context       // a context for the connection
}

func newRedisStorageHook(redisClient redis.UniversalClient) *RedisStorageHook {
	return &RedisStorageHook{db: redisClient}
}

// ID returns the id of the hook.
func (h *RedisStorageHook) ID() string {
	return "redis-db"
}

// Provides indicates which hook methods this hook provides.
func (h *RedisStorageHook) Provides(b byte) bool {
...
func (h *RedisStorageHook) hKey(s string) string {
	return h.config.HPrefix + s
}

// Init initializes and connects to the redis service.
func (h *RedisStorageHook) Init(config any) error {
	if _, ok := config.(*RedisOptions); !ok && config != nil {
...
func (h *RedisStorageHook) OnDisconnect(cl *mqtt.Client, _ error, expire bool) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) OnSubscribed(cl *mqtt.Client, pk packets.Packet, reasonCodes []byte) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) OnUnsubscribed(cl *mqtt.Client, pk packets.Packet) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) OnRetainMessage(cl *mqtt.Client, pk packets.Packet, r int64) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) OnQosPublish(cl *mqtt.Client, pk packets.Packet, sent int64, resends int) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) OnQosComplete(cl *mqtt.Client, pk packets.Packet) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) OnQosDropped(cl *mqtt.Client, pk packets.Packet) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
	}
...
func (h *RedisStorageHook) OnSysInfoTick(sys *system.Info) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) OnRetainedExpired(filter string) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) OnClientExpired(cl *mqtt.Client) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) StoredClients() (v []storage.Client, err error) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) StoredSubscriptions() (v []storage.Subscription, err error) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) StoredRetainedMessages() (v []storage.Message, err error) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) StoredInflightMessages() (v []storage.Message, err error) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
...
func (h *RedisStorageHook) StoredSysInfo() (v storage.SystemInfo, err error) {
	if h.db == nil {
		h.Log.Error("", "error", storage.ErrDBFileNotOpen)
		return
	}
